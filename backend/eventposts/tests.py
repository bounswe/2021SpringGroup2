...
class EventPostSearchTests(APITestCase):
    ...
    def test_filter_by_query(self):
        response = self.client.get(reverse('events-list'), {'query': 'basketball'}, HTTP_AUTHORIZATION=f'JWT {self.token}')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['results'], self.basketball_games)

    def test_filter_by_date(self):
        response = self.client.get(reverse('events-list'), {'min_date': datetime(2021, 12, 12),
    ...
    def test_filter_by_creation_date(self):
        response = self.client.get(reverse('events-list'), {'min_creation_date': (datetime.today() - timedelta(3)),
    ...
    def test_filter_by_player_capacity(self):
        response = self.client.get(reverse('events-list'), {'min_player_capacity': 6, 'max_player_capacity': 10},
                                   HTTP_AUTHORIZATION=f'JWT {self.token}')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['results'], self.player_capacity_between_6_10_games)

    def test_filter_by_spec_capacity(self):
        response = self.client.get(reverse('events-list'), {'min_spectator_capacity': 15, 'max_spectator_capacity': 25},
                                   HTTP_AUTHORIZATION=f'JWT {self.token}')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['results'], self.spec_capacity_between_15_25_games)

    def test_filter_by_location(self):
        response = self.client.get(reverse('events-list'), {'location': 'le'},
                                   HTTP_AUTHORIZATION=f'JWT {self.token}')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['results'], self.in_turkey_games)

    def test_filter_by_duration(self):
        response = self.client.get(reverse('events-list'), {'min_duration': 45, 'max_duration': 100},
                                   HTTP_AUTHORIZATION=f'JWT {self.token}')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['results'], self.duration_between_45_100_games)

    def test_filter_by_sport(self):
        response = self.client.get(reverse('events-list'), {'sport': 'Basketball'},
                                   HTTP_AUTHORIZATION=f'JWT {self.token}')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['results'], self.basketball_games)

    def test_filter_by_owner(self):
        response = self.client.get(reverse('events-list'), {'owner_id': self.user.id},
                                   HTTP_AUTHORIZATION=f'JWT {self.token}')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['results'], self.games_created_by_user)

    def test_filter_by_age(self):
        response = self.client.get(reverse('events-list'), {'min_age': 18, 'max_age': 35},
                                   HTTP_AUTHORIZATION=f'JWT {self.token}')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['results'], self.age_between_18_35_games)

    def test_filter_by_coordinates(self):
        response = self.client.get(reverse('events-list'), {'min_latitude': 36.23763062438484,
    ...
    def test_filter_by_player_numbers(self):
        response = self.client.get(reverse('events-list'), {'min_players': 1, 'max_players': 5},
                                   HTTP_AUTHORIZATION=f'JWT {self.token}')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['results'], self.num_players_between_1_5_games)

    def test_filter_by_spectator_numbers(self):
        response = self.client.get(reverse('events-list'), {'min_spectators': 3, 'max_spectators': 5},
                                   HTTP_AUTHORIZATION=f'JWT {self.token}')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['results'], self.num_spectators_between_3_5_games)

    def test_filter_by_skill(self):
        response = self.client.get(reverse('events-list'), {'min_skill_level': 2, 'max_skill_level': 4},
                                   HTTP_AUTHORIZATION=f'JWT {self.token}')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class EventPostApplicationTests(APITestCase):
    ...
    def test_apply_as_spectator(self):
        response = self.client.post(reverse('events-apply', args=[str(self.event_01.id)]),
    ...
    def test_apply_as_player_at_full_capacity(self):
        response = self.client.post(reverse('events-apply', args=[str(self.event_02.id)]),
                                    {'type': 'player', 'user': self.user.id},
                                    HTTP_AUTHORIZATION=f'JWT {self.token}')
    ...
    def test_apply_as_spectator_at_full_capacity(self):
        response = self.client.post(reverse('events-apply', args=[str(self.event_03.id)]),
    ...
    def test_apply_as_player_when_not_qualified(self):
        response = self.client.post(reverse('events-apply', args=[str(self.event_04.id)]),
                                    {'type': 'player', 'user': self.user.id},
                                    HTTP_AUTHORIZATION=f'JWT {self.token}')
    ...
    def test_accept_applicant_as_player(self):
        response = self.client.post(reverse('events-applicants', args=[str(self.event_04.id)]),
    ...
    def test_accept_applicant_as_player_at_full_capacity(self):
        response = self.client.post(reverse('events-applicants', args=[str(self.event_05.id)]),
                                    {'type': 'player', 'user': self.user4.id, 'owner': self.user.id, 'accept': True},
                                    HTTP_AUTHORIZATION=f'JWT {self.token}')
    ...
    def test_get_spec_applicants(self):
        response = self.client.get(reverse('events-applicants', args=[str(self.event_04.id)]), {'type': 'spectator'},
                                   HTTP_AUTHORIZATION=f'JWT {self.token}')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['applicants'], self.event_04.spec_applicants)

    def test_get_player_applicants(self):
        response = self.client.get(reverse('events-applicants', args=[str(self.event_04.id)]), {'type': 'player'},
                                   HTTP_AUTHORIZATION=f'JWT {self.token}')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
