...
class FragmentViewEventDetailed : Fragment(), RecyclerAdapterDiscussion.OnItemClickListener {
    ...
    private fun setClickListeners() {
        binding.btnEquipment.setOnClickListener {
            ...
            transaction.add(
                R.id.container_main,
                FragmentEquipmentResults.newInstance(
                    token,
                    username,
                    binding.tvEventTypeResult.text.toString()
                )
            ...
            ReboundAPI.create().getUser(token, username).enqueue(object : Callback<UserResponse> {
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        ...
                        if (request != null) {
                            ReboundAPI.create().applyToEvent(token, eventId, request)
                                .enqueue(object : Callback<EventbyIdResponse> {
                                    override fun onResponse(
                                        call: Call<EventbyIdResponse>,
                                        response: Response<EventbyIdResponse>
                                        ...
                                        if (response.isSuccessful) {
                                            Toast.makeText(
                                                requireContext(),
                                                "SUCCESSFULLY APPLIED AS PLAYER!",
                                                Toast.LENGTH_SHORT
                                            ).show()
                                        }
                                    }
                                ...
                                .enqueue(object : Callback<EventbyIdResponse> {
                                    override fun onResponse(
                                        ...
                                        if (response.isSuccessful) {
                                            Toast.makeText(
                                                requireContext(),
                                                "SUCCESSFULLY APPLIED AS SPECTATOR!",
                                                Toast.LENGTH_SHORT
                                            ).show()
                                        }
                                    }
        ...
        binding.btnPostComment.setOnClickListener {
            ...
            ReboundAPI.create().postComment(token, eventId, request)
                                ...
                                .enqueue(object : Callback<AllCommentsResponse> {
                                    override fun onResponse(
                                        ...
                                        if (response.isSuccessful) {
                                            ...
                                            layoutManager = LinearLayoutManager(context)
                                            binding.rvDiscussion.layoutManager = layoutManager
                                            adapter = RecyclerAdapterDiscussion(
                                                users,
                                                comments,
                                                dates,
                                                listener
        ...
        binding.btnPlayers.setOnClickListener {
            ...
            transaction.add(
                R.id.container_main,
                FragmentPlayers.newInstance(token, username, players)
            ).addToBackStack("equipmentResults")
            transaction.commit()
        }
                ...
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        ...
                        if (userId == ownerId) {
                            ...
                            Toast.makeText(
                                context,
                                "YOU NEED TO BE THE CREATOR OF THIS EVENT!!",
                                Toast.LENGTH_SHORT
                            ).show()
                        }
                    }
    ...
    companion object {
        ...
        fun newInstance(token: String, username: String, eventId: Int) =
            FragmentViewEventDetailed().also {
    ...
    override fun onItemClick(position: Int) {
        ...
        transaction.add(
            R.id.container_main,
            FragmentAnswers.newInstance(token, username, eventId, commentIds[position])
        ).addToBackStack("searchUser")
        transaction.commit()
    }
}