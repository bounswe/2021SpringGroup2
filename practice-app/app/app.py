...
def results(equipmentId):
    ...
    response = requests.get("https://rapidapi.p.rapidapi.com/api/v1/search/" + urllib.parse.urlencode(query),
                            headers=headers)
    mapped = [{"description": j["description"], "link": j["link"], "title": j["title"]} for j in
...
def get_sport_types():
    try:
        ...
        mapped = [{"name": j["attributes"]["name"], "description": j["attributes"]["description"],
                   "icon": j["attributes"]["icon"]} for j in response.json()["data"] if
                  j["attributes"]["decathlon_id"] != None and j["attributes"]["parent_id"] == None]
        return jsonify(mapped), 200
    except:
        abort(500)
...
def get_weather(city, year, month, day):
    try:
        ...
        result = requests.get(
            "https://www.metaweather.com/api/location/" + str(id) + "/" + str(year) + "/" + str(month) + "/" + str(day))
        mapped = {key: result.json()[0][key] for key in
                  ["humidity", "max_temp", "min_temp", "the_temp", "weather_state_name", "wind_speed"]}
    ...
    for argument in request.args:
        ...
        if argument == 'address' or argument == 'radius' or argument == 'ip':
            ...
            if request.args["empty"] == "true":
                filteredEvents = list(
                    filter(lambda event: len(event["players"]) < int(event["playerCapacity"]), filteredEvents))
        else:
